...
Standard brand profile inheriting directly from system parent:
```
...
```
System parent (e.g. Anycubic PLA @Anycubic Kobra S1 0.4 nozzle)
    └── Improved PLA @AC KS1 0.4mm   ← user calibration base
            └── EconoFil PLA @AC KS1 0.4mm  ← brand overlay (inherits Improved, not system)
                    ├── EconoFil PLA @AC KS1 0.25mm
                    ├── EconoFil PLA @AC KS1 0.6mm
                    └── EconoFil PLA @AC KS1 0.8mm
```

Key rule: Standard PLA brands should inherit from `Improved PLA @AC KS1/KX 0.4mm` (not directly from the system parent) because Improved PLA carries the calibrated retraction, z-hop, fan, and temperature settings. Only the brand-specific differences need to be in the brand overlay.

KX chain is identical but rooted in KX system parent and KX user 0.4mm.
...
### Application Notes
- 0.25mm MVS: ×0.50 of 0.4mm value, always capped at 3 mm³/s maximum
